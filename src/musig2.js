...
const pointToBytes = schnorr.utils.pointToBytes; // 32-byte x-only
...
export function nonceAgg(pubNonces) {
  ...
  for (let j = 0; j < 2; j++) {
    ...
    for (let i = 0; i < pubNonces.length; i++) {
      Rj = Rj.add(Point.fromBytes(pubNonces[i].slice(j * 33, j * 33 + 33)));
    }
    aggR.push(Rj);
  }
...
export function partialSign(secretKey, secNonce, aggNonce, keyCoeffs, aggPubkey, msg, signerIndex, gacc) {
  ...
  // BIP-327: d = g * gacc * d_raw
  // Since we always normalize Q to even-Y in keyAgg, g = 1.
  // But we use x-only pubkeys, so lift_x always gives even-Y.
  // The signer's P = d_raw * G might have odd Y.
  // lift_x(pk) = P if even-Y, else -P = (-d_raw)*G
  // So effective secret relative to lift_x is:
  //   d_eff = d_raw if P has even Y, else -d_raw
  // Then: d = gacc * d_eff
...
export function partialSigVerify(partialSig, pubNonce, pubkey, aggNonce, keyCoeffs, aggPubkey, msg, signerIndex, gacc) {
  ...
  // The signer's key as used in the aggregate: gacc * lift_x(pk)
  // (gacc accounts for aggregate key negation)
